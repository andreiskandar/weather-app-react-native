...
export default function WeatherDetails({ currentWeather, unit }) {
  ...
  const windSpeed = unit === 'metric' ? `${Math.round(speed)} m/s` : `${Math.round(speed)}mph`;
  return (
    <View style={styles.weatherDetails}>
      <View style={styles.weatherDetailsRow}>
        ...
        <View style={{ ...styles.weatherDetailsBox, borderRightWidth: 1, borderRightColor: BORDER_COLOR }}>
          <View style={styles.weatherDetailsRow}>
            <MaterialCommunityIcons name='water' size={30} color={PRIMARY_COLOR}></MaterialCommunityIcons>
            <View style={styles.weatherDetailsItem}>
              <Text>Humidity:</Text>
